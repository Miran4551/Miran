...
export const WorkspaceOverview: React.FC<{ onNavigate: (tab: string) => void }> = ({ onNavigate }) => {
  ...
  // "المتدربون الحاليون" means trainees actually active at this hospital now,
  // not every historical/legacy profile whose organizationId happens to point to
  // the hospital. The incoming endpoint is the operational placement source and
  // its live request-row status is preferred over applicationStatus.
  const { data: incomingTrainees } = useQuery({
    queryKey: ['ws-active-trainees', orgId],
  ...
  const currentTraineeCount = currentTraineeIds.size;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: space['2xl'] }}>
      <KpiGrid>
        ...
        <KpiCard label="المتدربون الحاليون" value={currentTraineeCount} icon={Stethoscope} tone="success"
