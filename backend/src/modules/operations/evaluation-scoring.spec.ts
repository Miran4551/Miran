 ...
 * 4. Multiple forms may be submitted in the same evaluation stage
 */
describe('Evaluation — criterion scoring + ownership', () => {
  ...
  function makeService(opts: {
    ...
    formItems?: unknown[];
    hasDeptEval?: boolean;
    existingTrainerEvalFormId?: string;
    isLocked?: boolean;
  }) {
    const prisma = {
      rotation: {
      ...
      evaluation: {
        findFirst: jest.fn().mockImplementation(({ where }: any) => {
          if (where?.evaluationType === 'department_by_trainee') {
            return Promise.resolve(opts.hasDeptEval ? { id: 'dept-eval-1' } : null);
          }
          if (opts.existingTrainerEvalFormId && where?.formId === opts.existingTrainerEvalFormId) {
            return Promise.resolve({ id: 'existing-trainer-eval-1', submittedAt: new Date() });
          }
  ...
  // ─── Test 4: Different forms in the same stage are allowed ──────────────

  it('allows another evaluation form for the same trainee and stage', async () => {
    const { service, prisma } = makeService({
      callerTrainerProfileId: TRAINER_PROFILE_A,
      rotationTrainerProfileId: TRAINER_PROFILE_A,
      rotationTraineeProfileId: 'trainee-profile-A',
      ...
      midpointMeetingDone: true,
      hasDeptEval: true,
      existingTrainerEvalFormId: 'form-other',
    });
    ...
    expect(prisma.evaluation.findFirst).toHaveBeenCalledWith({
      where: {
        rotationId: ROTATION_ID,
        evaluatorId: trainerUserA.accountId,
        evaluateeId: TRAINEE_ACCOUNT_A,
        evaluationType: 'final_rotation',
        formId: FORM_ID,
      },
  ...
  // ─── Test 5: Score exceeding max is rejected ────────────────────────────
