  ...
  async findAll(
    ...
    if (scope.visibleOrgIds !== null) {
      where.OR = [
        ...
        // Legacy/university-originated requests can carry the receiving cluster
        // only through the academic-intake record. Keep the list scope aligned
        // with the ownership evidence already accepted by ScopeGuard.
        { academicIntake: { is: { organizationId: { in: scope.visibleOrgIds } } } },
        // Imported trainee rows can preserve the sponsoring university and the
        // eventual receiving hospital even when the top-level request pointers
        // are stale or were created before those fields were populated.
        { trainees: { some: { universityOrgId: { in: scope.visibleOrgIds } } } },
        { trainees: { some: { assignedHospitalId: { in: scope.visibleOrgIds } } } },
      ];
    }
