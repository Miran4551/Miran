      ...
      case 'trainingRequest': {
        const r = await this.prisma.trainingRequest.findUnique({
          ...
          select: {
            sourceOrgId: true,
            targetOrgId: true,
            // The academic intake and trainee rows are additional ownership
            // evidence for legacy requests created before sourceOrgId was
            // consistently populated. A university must be able to reopen a
            // request it owns even when an older row has a stale/missing source
            // organisation pointer.
            academicIntake: { select: { organizationId: true } },
            trainees: { select: { assignedHospitalId: true, universityOrgId: true }, distinct: ['assignedHospitalId', 'universityOrgId'] },
        ...
        const universityIds = r.trainees.map((t) => t.universityOrgId).filter((o): o is string => !!o);
        const intakeOrgId = r.academicIntake?.organizationId;
        return [
          r.sourceOrgId,
          r.targetOrgId,
          intakeOrgId,
